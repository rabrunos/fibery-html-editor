...
* `source/html/template-resources-host.html` — `<div id="templateResourcesHost" style="display:contents;">` host element for runtime template injection; replaces the 8 externalized modal HTML files in the inline build.
...
* `source/js/app-style-resources.ts` — new module for cached CSS injection: `styleResourceElementId`, `injectCachedStyle`, `applyCachedStyleResource`, `ensureCachedStyleResourcesLoaded`. Injects CSS from `appResources` IndexedDB as a `<style>` element after validation; idempotent (updates existing element if re-applied).
* `support/8.43.0/css/app-noncritical.css` — versioned external CSS resource containing non-critical styles: `mini-tooltip`, `draft-diff-*`, all Update App/changelog styles, and `@keyframes miniTooltip`. Registered as `css/app-noncritical` in the resource manifest with `kind: "style"`, `required: true`, sha256, and versionated URL.
* `support/8.43.0/resources-manifest.json` — includes `i18n/en`, `i18n/pt-BR` (required), `update/changelog` (not required), and `css/app-noncritical` (required).
* i18n JSON resources propagated to `support/8.43.0/i18n/`.

### Technical adjustments

* `source/css/02-preview-and-diff.css` reduced to 4 critical panel-mode rules (`.panel-editor-only`, `.panel-preview-only`); these classes are applied at boot via `setPanelMode()` before the DB opens, so they cannot be loaded from IndexedDB in time and must remain inline.
* `source/css/03-update-panel.css` and `source/css/04-animations.css` removed from the manifest's inline `css` array; their content moved to `support/8.43.0/css/app-noncritical.css`. Source files preserved as references.
* `lifecycle-init.ts` updated: `ensureCachedStyleResourcesLoaded()` called after `ensureRequiredResources()` and before `ensureI18nResourcesLoaded()`.
* `app-resources-loader.ts` updated: after successful retry download, `ensureCachedStyleResourcesLoaded()` is also called so CSS is applied immediately after overlay dismissal.
* `scripts/checks.mjs` updated: new `checkCssExternalization()` validates that externalized CSS files are absent from the inline manifest, that critical CSS files remain, and that `update/changelog` resource is `required: false`.
* `scripts/validate-build.mjs` updated: `ensureCachedStyleResourcesLoaded` added to bootstrap symbol checks; assertion added that no `<link>` element references the external CSS file.
...
* On first run with no cache, the overlay downloads `i18n`, `css/app-noncritical`, and any other required resources together. After download, CSS is injected automatically and the full visual is applied.
* The inline CSS covers all boot-critical styles: full layout, sidebar states, panel modes, context menus, inline rename, and preview-focus mode.
...
* `source/js/update-changelog-resource.ts` — new module integrating the Update App changelog with the `appResources` IndexedDB cache. Provides `getCachedUpdateChangelog`, `cacheUpdateChangelogFromRemote`, `loadUpdateChangelogWithCache`, and `refreshUpdateChangelogResource`.
* `support/8.42.0/changelog.md` — versioned snapshot of CHANGELOG.md for this release, registered in the resource manifest as a non-required `data` resource with sha256 and versionated URL.
* `support/8.42.0/resources-manifest.json` — includes `i18n/en`, `i18n/pt-BR` (required), and `update/changelog` (not required).
...
* Changelog resource key is `update/changelog`; kind is `data`; `required: false` — changelog cache failure never blocks boot or version check.
* No functional JavaScript externalized; all runtime scripts remain inline in `index.html`.
* Vite single-file build preserved; deploy artifact remains a single `index.html`.
* Fibery runtime tests remain manual.

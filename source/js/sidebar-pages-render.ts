function appPageBadge(pageId: string): string {
  ...
  return `<span class="ml-1 inline-flex h-5 w-5 shrink-0 self-center items-center justify-center rounded-md bg-yellow-100 text-yellow-800 ring-1 ring-yellow-200" title="${escapeHtml(t('appPageMarker'))}">${iconCode({ className: 'h-3.5 w-3.5' })}</span>`;
}

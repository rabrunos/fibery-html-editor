...
function validateRemoteUpdateHtml(remoteHtml: string = ''): RemoteUpdateValidationResult {
  ...
  if (appVersionLiteral !== metaSemver.raw) {
    return { ok: false, reason: 'version-mismatch', metaVersion: metaSemver.raw, appVersionLiteral };
  }

  if (!/window\.FIBERY_HTML_EDITOR_VERSION\s*=\s*APP_VERSION/.test(source)) {
    return { ok: false, reason: 'missing-window-version-assign', metaVersion: metaSemver.raw };
  }
  if (!/document\.documentElement\.dataset\.appVersion\s*=\s*APP_VERSION/.test(source)) {
    return { ok: false, reason: 'missing-dataset-version-assign', metaVersion: metaSemver.raw };
  }

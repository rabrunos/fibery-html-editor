...
async function loadWelcomeSearchResults(options = {}) {
  ...
  try {
    ...
    if (!localRows.length) els.welcomeSearchResults.innerHTML = `<div class="p-4 text-center text-sm text-red-500">${escapeHtml(err.message || err)}</div>`;

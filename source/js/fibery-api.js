const API = {
  ...
  async loadPages({ skip = 0, limit = 21, search = '', source = 'unknown', automatic = false } = {}) {
  ...
  async savePage(page, options = {}) {
    return withApiUsage({ kind: 'fibery-api', operation: 'savePage', source: options.source || 'save-page', pageId: page?.id || '', automatic: !!options.automatic }, async () => {
      ...
      const r = await fetch(isUpdate ? '/api/ai-answer/pages/' + encodeURIComponent(page.id) : '/api/ai-answer/pages/', { method: isUpdate ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(page) });
      if (!r.ok) { let msg = 'Failed to save page'; try { msg += ': ' + JSON.stringify(await r.json()); } catch (_) {} throw new Error(msg); }
      return r.json();
    });
  },
